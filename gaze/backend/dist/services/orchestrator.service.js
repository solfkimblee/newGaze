...
export async function orchestrate(input) {
    ...
    if (/图像|图片|看/.test(user) || /可视化|画像|相貌/.test(user)) {
        return {
            action: 'VISUALIZE',
            responseText: '我能感受到一个需要被视觉化呈现的意象。我来为你描绘它的轮廓。',
            image_context: 'Ethereal, mystical aura, a calm East Asian figure with warm eyes, soft bokeh background, symbolic charm glowing',
    ...
    return {
        action: 'INSIGHT',
        responseText: '你的叙述显示能量在回归中心。请在接下来的三天专注一件对你重要的小事。',
        next_ui_component: 'UI_TextInput'
    };
}
